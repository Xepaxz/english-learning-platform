  ...
  return (
    <div className="py-14 md:py-24 bg-white min-h-screen">
      <div className="container mx-auto px-4 md:px-6">
        ...
        >
          <h1 className="text-3xl sm:text-4xl md:text-5xl font-bold font-serif mb-4 md:mb-6">Learning Programs</h1>
        ...
        <div className="grid md:grid-cols-3 gap-5 md:gap-8 max-w-6xl mx-auto">
          {courses.map((course, index) => (
            <AnimatedCard
              key={index}
              delay={index * 120}
              className={`rounded-2xl md:rounded-3xl p-6 md:p-8 border flex flex-col ${course.color} hover:-translate-y-2 hover:shadow-xl duration-300`}
            >
              <h3 className="text-xl md:text-2xl font-serif font-bold mb-2">{course.title}</h3>
              <div className="text-3xl md:text-4xl font-bold text-primary mb-4 md:mb-6">
                {course.price}<span className="text-base md:text-lg text-muted-foreground font-normal">/month</span>
              </div>
              <p className="text-foreground/80 mb-6 md:mb-8 flex-grow leading-relaxed text-sm md:text-base">{course.description}</p>
        ...
        <div
          ...
          className={`mt-16 md:mt-24 text-center transition-all duration-700 ease-out ${ctaIn ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"}`}
        >
          <h3 className="text-xl md:text-2xl font-serif font-bold mb-3 md:mb-4">Not sure which level is right for you?</h3>
          <p className="text-muted-foreground mb-6 md:mb-8 text-sm md:text-base">Schedule a free 15-minute consultation to assess your level.</p>
